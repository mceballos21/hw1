//modified by:Marisela Ceballos
//date:5/4/17
//purpose:HW 1- waterfall SDLC
//
...
Shape *s = &game->box; 
	...
	if(p->s.center.y < s->center.y-110 + s->height &&
		...
		p->s.center.x < s->center.x+125 + s->width)
	{
	    p->s.center.y = s->center.y-110 + s->height;
	    p->velocity.y = -p->velocity.y;
	    p->velocity.y *= 0.5;
		...
		p->s.center.x < s->center.x-200 + s->width)
	{
	    ...
	    p->s.center.y = s->center.y+180 + s->height;
	    p->velocity.y = -p->velocity.y;
	    p->velocity.y *= 0.5;
	...
	if(p->s.center.y < s->center.y+280 + s->height &&
		...
		p->s.center.x < s->center.x-350 + s->width)
	{
	    p->s.center.y = s->center.y+280 + s->height;
	    p->velocity.y = -p->velocity.y;
	    p->velocity.y *= 0.5;
...
void render(Game *game)
{
    ...
    s = &game->box;
    glPushMatrix();
    glTranslatef(s->center.x-75, s->center.y+120, s->center.z);
    w = s->width;
    h = s->height;
    glBegin(GL_QUADS);
    glVertex2i(-w,-h);
    ...
    s2 = &game->box;
    glPushMatrix();
    glTranslatef(s2->center.x+40, s2->center.y, s2->center.z);
    w = s2->width;
    h = s2->height;
    glBegin(GL_QUADS);
    glVertex2i(-w,-h);
    ...
    s3 = &game->box;
    glPushMatrix();
    glTranslatef(s3->center.x+125, s3->center.y-110, s3->center.z);
    w = s3->width;
    h = s3->height;
    glBegin(GL_QUADS);
    glVertex2i(-w,-h);
    ...
    s4 = &game->box;
    glPushMatrix();
    glTranslatef(s4->center.x-200, s4->center.y+180, s4->center.z);
    w = s4->width;
    h = s4->height;
    glBegin(GL_QUADS);
    glVertex2i(-w,-h);
    ...
    s5 = &game->box;
    glPushMatrix();
    glTranslatef(s5->center.x-350, s5->center.y+280, s5->center.z);
    w = s5->width;
    h = s5->height;
    glBegin(GL_QUADS);
    glVertex2i(-w,-h);
